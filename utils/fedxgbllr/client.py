...
from .models import CNN, fit_xgboost
from .utils import single_tree_preds_from_each_client
...
class FlClient(fl.client.Client):
    ...
    def get_parameters(self, ins):
        ...
        tree = fit_xgboost(
            self.config, self.config.dataset.task.task_type, data, label, self.config.n_estimators_client
        )
    ...
    def fit(self, ins: FitIns) -> FitRes:
        ...
        self.net.set_weights(
            parameters_to_ndarrays(ins.parameters[0]))  # type: ignore # noqa: E501 # pylint: disable=line-too-long
        aggregated_trees = ins.parameters[1]  # type: ignore # noqa: E501 # pylint: disable=line-too-long

        if isinstance(aggregated_trees, list):
            print("Client " + self.cid + ": received", len(aggregated_trees), "XGBoost models")

            # save the aggregated trees in a folder in the same directory, called aggregated_trees/client_id/model_round_id
            current_dir = os.path.dirname(__file__)
            path = os.path.join(current_dir, "aggregated_trees")
            if not os.path.exists(path):
                os.makedirs(path)
            path = os.path.join(path, self.cid)
            if not os.path.exists(path):
                os.makedirs(path)
            with open(os.path.join(path, f"model_{self.saving_round}.pkl"), "wb") as f:
                pickle.dump(aggregated_trees, f)
