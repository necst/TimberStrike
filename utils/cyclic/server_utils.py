...
def evaluate_metrics_aggregation(eval_metrics):
    ...
    metrics_aggregated = {"AUC": auc_aggregated, "F1": f1_aggregated}
    print(f"Aggregated AUC: {auc_aggregated}, F1: {f1_aggregated}, total_num: {total_num}")
...
def get_evaluate_fn(test_data):
    ...
    def evaluate_fn(
        ...
        if server_round == 0:
            ...
            f1 = f1_score(
                test_data.get_label(),
                bst.predict(test_data) > 0.5,
            )

            log(INFO, f"AUC = {auc} at round {server_round}")

            return 0, {"AUC": auc, "F1": f1}
