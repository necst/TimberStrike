...
class GrpcServerAdaptor(XGBServerAdaptor):
    ...
    def all_gather(self, rank: int, seq: int, send_buf: bytes, fl_ctx: FLContext) -> bytes:
        print(f"all_gather: rank={rank}, seq={seq}")
    ...
    def all_gather_v(self, rank: int, seq: int, send_buf: bytes, fl_ctx: FLContext) -> bytes:
        print(f"all_gather_v: rank={rank}, seq={seq}")
    ...
    def all_reduce(
        ...
        print(f"all_reduce: rank={rank}, seq={seq}, data_type={data_type}, reduce_op={reduce_op}")
        type_map = {
            pb2.FLOAT: (np.float32, 'float'),
            pb2.DOUBLE: (np.double, 'double'),
            pb2.UINT32: (np.uint32, 'uint32'),
            pb2.UINT64: (np.uint64, 'uint64'),
        }
        ...
        os.makedirs("all_reduce", exist_ok=True)
        os.makedirs(f"all_reduce/all_reduce{rank}", exist_ok=True)

        with open(f"all_reduce/all_reduce{rank}/all_reduce{seq}.txt", "w") as f:
            converted = np.frombuffer(send_buf, dtype=type_map[data_type][0])
            len_converted = len(converted)
            f.write(
                f"num_bytes: {len(send_buf)}, num_elements: {len_converted}, data_type: {type_map[data_type][1]}, reduce_op: {reduce_map[reduce_op]}, array: {converted}\n")
        with open(f"all_reduce/all_reduce{rank}/all_reduce{seq}.npy", "wb") as f1:
            np.save(f1, converted)
        ...
        os.makedirs("all_reduce_results", exist_ok=True)
        with open(f"all_reduce_results/all_reduce_results{seq}.txt", "w") as f:
            converted = np.frombuffer(result.receive_buffer, dtype=type_map[data_type][0])
            len_converted = len(converted)
            f.write(
                f"num_bytes: {len(result.receive_buffer)}, num_elements: {len_converted}, data_type: {type_map[data_type][1]}, reduce_op: {reduce_map[reduce_op]}, array: {converted}\n")
        with open(f"all_reduce_results/all_reduce_results{seq}.npy", "wb") as f1:
            np.save(f1, converted)
    ...
    def broadcast(self, rank: int, seq: int, root: int, send_buf: bytes, fl_ctx: FLContext) -> bytes:
        print(f"broadcast: rank={rank}, seq={seq}, root={root}")
