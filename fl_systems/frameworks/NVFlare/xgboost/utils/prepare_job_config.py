...
BASE_JOB_MAP = {"histogram": "base", "histogram_v2": "base_v2"}


def job_config_args_parser():
    parser = argparse.ArgumentParser(description="generate train configs for dataset")
    parser.add_argument(
        "--data_root",
        type=str,
        default="/tmp/nvflare/xgboost_dataset",
    ...
    parser.add_argument("--base", type=str, default=None, help="Base job config to use")
    parser.add_argument("--lambda", type=float, default=1.0, help="Regularization parameter", dest="lambda_")
    ...
    parser.add_argument("--max_depth", type=int, default=7, help="Maximum depth of the tree")
    return parser
...
def _get_data_split_name(args, site_name: str) -> str:
    return os.path.join(args.data_root, f"{args.site_num}_clients", f"data_{site_name}.json")



def _get_src_job_dir(args, training_mode):
    first_part = XGB_EXAMPLE_ROOT / JOB_CONFIGS_ROOT  # XGB_EXAMPLE_ROOT / MODE_ALGO_MAP[training_mode] / JOB_CONFIGS_ROOT
    c = BASE_JOB_MAP[training_mode] if args.base is None else args.base
...
def _update_client_config(config: dict, args, site_name: str):  # , lr_scale):
    ...
    if args.training_mode == "bagging" or args.training_mode == "cyclic":
        ...
        config["executors"][0]["executor"]["args"]["xgb_params"]["lambda"] = args.lambda_
        config["executors"][0]["executor"]["args"]["xgb_params"]["gamma"] = args.gamma
        config["executors"][0]["executor"]["args"]["xgb_params"]["eta"] = args.eta
        config["executors"][0]["executor"]["args"]["xgb_params"]["max_depth"] = args.max_depth
...
def _update_server_config(config: dict, args):
    ...
    for i in range(1, args.site_num + 1):
        client_ranks[f"{args.site_name_prefix}{i}"] = i - 1
    config["workflows"][0]["args"]["client_ranks"] = client_ranks
    config["num_rounds"] = args.round_num
...
def create_client_app(src_job_path, src_app_name, dst_job_path, site_name, args):
    ...
    _update_client_config(client_config, args, site_name) # , lr_scales[site_name])
...
def main():
    ...
    src_job_path = _get_src_job_dir(args, args.training_mode)

    # create a new job
    dst_job_path = XGB_EXAMPLE_ROOT / JOB_CONFIGS_ROOT / job_name  # XGB_EXAMPLE_ROOT / MODE_ALGO_MAP[args.training_mode] / JOB_CONFIGS_ROOT / job_name
