...
void Tree::preorder_traversal(int nid, int max_depth, int depth, string &s) const {
    ...
    if (node.is_valid && !node.is_pruned) {
        ...
        if (node.is_leaf) {
            s = s + string_format("%d:leaf=%.6g,sum_gh_pair=(%.6g,%.6g),gain=%.6g\n",
                                  node.final_id,
                                  node.base_weight,
                                  node.sum_gh_pair.g, node.sum_gh_pair.h,
                                  node.gain
                );
        } else {
            int lch_final_id = node_data[node.lch_index].final_id;
            int rch_final_id = node_data[node.rch_index].final_id;
            string str_inter_node = string_format(
                "%d:[f%d<%.6g] yes=%d,no=%d,missing=%d,sum_gh_pair=(%.6g,%.6g),gain=%.6g\n",
                node.final_id,
