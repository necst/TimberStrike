...
// float_type BinaryClassMetric::get_auc_score(const SyncArray<float_type> &y_p) const {
//     /*
//     // compute test error
//     int n_instances = y.size();
//     auto y_data = y.host_data();
...
float_type BinaryClassMetric::get_score(const SyncArray<float_type> &y_p) const {
    int n_instances = y.size();
    auto y_data = y.host_data();  // true labels
    auto yp_data = y_p.host_data();  // predicted labels

    int tp = 0, fp = 0, tn = 0, fn = 0;

    // Iterate over each instance and count outcomes.
    // Here, a threshold of 0.5 is used to determine class membership.
    for (int i = 0; i < n_instances; i++) {
        bool true_label = (y_data[i] >= 0.5);
        bool pred_label = (yp_data[i] >= 0.5);
    ...
    // If both precision and recall are zero, return 0 to avoid division by zero.
