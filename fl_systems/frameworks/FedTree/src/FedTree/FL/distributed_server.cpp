...
grpc::Status DistributedServer::GetGradients(grpc::ServerContext *context, const fedtree::PID *id,
    ...
    LOG(INFO) << "GetGradients";
    // print pid
    LOG(DEBUG) << "pid: " << id->id() << " get gradients" << std::endl;
...
DistributedServer::SendHistograms(grpc::ServerContext *context, grpc::ServerReader<fedtree::GHPair> *reader,
                                  fedtree::PID *id) {
    LOG(DEBUG) << "SendHistograms";
    LOG(DEBUG) << "pid: " << id->id() << " send histograms";
    fedtree::GHPair hist;
    std::multimap<grpc::string_ref, grpc::string_ref> metadata = context->client_metadata();
...
grpc::Status DistributedServer::SendRange(grpc::ServerContext* context, grpc::ServerReader<fedtree::GHPair>* reader,
    ...
    // save gh into a file
    // std::ofstream file;
    // file.open("server_messages/range_" + std::to_string(pid) + ".txt");
    // for (auto e: party_feature_range[pid]) {
    //     file << e.g << " " << e.h << std::endl;
    // }
    // file.close();
...
grpc::Status DistributedServer::SendGH(grpc::ServerContext* context, const fedtree::GHPair* request,
    ...
    // save gh into a file
    // std::ofstream file;
    // file.open("server_messages/gh_" + std::to_string(pid) + ".txt", std::ios::app);
    // file << request->g() << " " << request->h() << std::endl;
    // file.close();
    ...
    std::default_random_engine generator(seed);
    std::uniform_int_distribution<int> delay_distribution(5, 10);
    while (true) { // wait for all parties to send their gh
        bool cont = true;
        for (int i = 0; i < param.n_parties; i++) {
...
grpc::Status DistributedServer::TriggerCalcTree(grpc::ServerContext* context, const fedtree::PID* request, fedtree::Ready* response) {
    ...
    LOG(DEBUG) << "[hist_received/missing_gh_received/hist_fid_received]: " << hists_received << "/" << missing_gh_received << "/" << hist_fid_received;
    {
    ...
    SyncArray <GHPair> hist;
    {
        ...
        LOG(DEBUG) << "Merging with merge_histograms_server_propose";
...
grpc::Status DistributedServer::SendHistFidBatches(grpc::ServerContext *context, grpc::ServerReader<fedtree::FIDBatch> *reader,
    ...
    // LOG(DEBUG) << "Receive hist_fid from " << pid;
    // // Save the received hist_fid into a file
    // if(pid == 0){
    //     std::ofstream hist_fid_file;
    //     hist_fid_file.open("server_messages/hist_fid.txt");
    //     for(int i = 0; i < len; i++){
    //         hist_fid_file << hist_data[i] << std::endl;
    //     }
    //     hist_fid_file.close();
    // }
...
grpc::Status DistributedServer::StopServer(grpc::ServerContext *context, const fedtree::PID *request,
    ...
    if (request->id() == 0) {
        ...
        // Perform shutdown in a separate thread
        std::thread shutdown_thread([this] {
            const auto deadline = std::chrono::system_clock::now() + std::chrono::seconds(5);
            this->grpc_server_->Shutdown(deadline);
            LOG(INFO) << "Shutdown initiated. Waiting for server to finish";
