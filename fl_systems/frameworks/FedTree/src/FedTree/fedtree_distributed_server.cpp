...
int main(int argc, char **argv) {
    ...
    if (fl_param.mode == "vertical") {
        ...
        LOG(INFO) << "horizontal partitioning";
    ...
    // Initialize grpc_server as a shared pointer directly
    const std::shared_ptr<grpc::Server> grpc_server = builder.BuildAndStart();
    server.setGrpcServer(grpc_server); // Set the grpc_server reference in DistributedServer
