...
class FedXgbBagging(FedAvg):
    ...
    def aggregate_fit(
        ...
        for proxy, fit_res in results:
            log(INFO, f"Results from {proxy.cid} received")
            cid = fit_res.metrics["cid"]

            current_dir = os.path.dirname(os.path.abspath(__file__))
            cid_path = os.path.join(current_dir, "cids")
            os.makedirs(cid_path, exist_ok=True)
            if os.path.exists(os.path.join(cid_path, "cid_list.pkl")):
                with open(os.path.join(cid_path, "cid_list.pkl"), "rb") as f:
                    cids = pickle.load(f)
            else:
                cids = []
            cids.append(cid)
            with open(os.path.join(cid_path, "cid_list.pkl"), "wb") as f:
                pickle.dump(cids, f)

            update = fit_res.parameters.tensors
            # to_save.append({"cid": proxy.cid, "tree": update})
            for bst in update:
                global_model = aggregate(global_model, bst)

        # with open("global_model/saved_models.pkl", "ab") as f:
