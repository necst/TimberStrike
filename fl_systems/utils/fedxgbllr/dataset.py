...
def load_partitions(dataset_name: str, pool_size: int, batch_size: int, data_path: str) -> Tuple[
    list[DataLoader[tuple[Tensor, ...]]], list[DataLoader[tuple[Tensor, ...]]], DataLoader[tuple[Tensor, ...]]]:
    """Load partitions of the dataset.
    ...
    Returns
    -------
        Tuple[List[DataLoader], List[DataLoader]]: A tuple containing the training
        and test loaders.
    """
    # if dataset_name != "stroke" and dataset_name != "breast" and dataset_name != "diabetes":
    #     raise ValueError("Not found partitions for the specified dataset")
    ...
    print("Loading partitions for the dataset: stroke")
    print(f"Data path: {data_path}")
    print(f"Number of clients: {pool_size}")
    print(f"Batch size: {batch_size}")

    # Load data from each client
    for i in range(pool_size):
        x_train = torch.from_numpy(np.load(f"{data_path}/client_{i}/train/x_train.npy"))
        y_train = torch.from_numpy(np.load(f"{data_path}/client_{i}/train/y_train.npy"))
        x_valid = torch.from_numpy(np.load(f"{data_path}/client_{i}/valid/x_valid.npy"))
        y_valid = torch.from_numpy(np.load(f"{data_path}/client_{i}/valid/y_valid.npy"))
        batch_size_train = batch_size if batch_size != "whole" else len(x_train)
        batch_size_valid = batch_size if batch_size != "whole" else len(x_valid)
        trainloaders.append(DataLoader(TensorDataset(x_train, y_train), batch_size=batch_size_train, shuffle=True))
        valloaders.append(DataLoader(TensorDataset(x_valid, y_valid), batch_size=batch_size_valid, shuffle=False))

        # concatenate all the test data
        if x_test is None:
            x_test = torch.from_numpy(np.load(f"{data_path}/client_{i}/valid/x_valid.npy"))
            y_test = torch.from_numpy(np.load(f"{data_path}/client_{i}/valid/y_valid.npy"))
        else:
            x_test = torch.cat((x_test, torch.from_numpy(np.load(f"{data_path}/client_{i}/valid/x_valid.npy"))),
                               dim=0)
            y_test = torch.cat((y_test, torch.from_numpy(np.load(f"{data_path}/client_{i}/valid/y_valid.npy"))),
                               dim=0)
    ...
    batch_size_test = batch_size if batch_size != "whole" else len(x_test)
    testloader = DataLoader(TensorDataset(x_test, y_test), batch_size=batch_size_test, shuffle=False)
