...
def test(
    ...
    total_loss, total_result, total_result_auc, n_samples = 0.0, 0.0, 0.0, 0
    net.eval()
    with torch.no_grad():
        ...
        for data in tqdm(testloader, desc="TEST") if log_progress else testloader:
            ...
            if cfg.dataset.task.task_type == "BINARY":
                # use torchmetrics to calculate AUC, DO NOT USE METRIC_FN
                total_result_auc += binary_auroc(outputs.cpu(), labels.type(torch.int).cpu()) * labels.size(0)


    if log_progress:
        print("\n")

    return total_loss / n_samples, total_result / n_samples, total_result_auc / n_samples, n_samples
