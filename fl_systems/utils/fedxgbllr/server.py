...
from .utils import EarlyStop, single_tree_preds_from_each_client, test
...
def serverside_eval(
    ...
    loss, result, auc_result, _ = test(cfg, model, testloader, device=device, log_progress=False)

    print(
        f"Evaluation on the server: test_loss={loss:.4f},",
        f"test_{metric_name}={result:.4f}",
        f"test_AUC={auc_result:.4f}",
    )
