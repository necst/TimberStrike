...
class XGBClientRunner(AppRunner, FLComponent):
    ...
    def _xgb_train(self, params: XGBoostParams, train_data: xgb.DMatrix, val_data) -> xgb.core.Booster:
        ...
        def f1_eval(preds, dtrain):
            ...
            # Calculate F1 score with a safeguard against division by zero
            f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0.0

            return 'f1', f1

        # Example usage in the training call:
        bst = xgb.train(
            params.xgb_params,
            train_data,
            params.num_rounds,
            ...
            callbacks=callbacks,
            custom_metric=f1_eval  # Use f1_eval as the custom evaluation metric
        )
        return bst
    ...
    def run(self, ctx: dict):
        ...
        with xgb.collective.CommunicatorContext(**communicator_env):
            ...
            print(f"Saving model to {os.path.join(self._model_dir, self.model_file_name)}")
