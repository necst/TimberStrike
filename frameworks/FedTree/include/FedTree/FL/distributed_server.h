...
public:
    ...
    void setGrpcServer(const std::shared_ptr<grpc::Server>& server) {
        grpc_server_ = server;
    }

    // Add a getter if you need to access grpc_server from other parts of the code
    std::shared_ptr<grpc::Server> getGrpcServer() const {
        return grpc_server_;
    }
