...
void HistTreeBuilder::get_best_gain_in_a_level(SyncArray<float_type> &gain, SyncArray<int_float> &best_idx_gain,
    ...
    int n_split = n_nodes_in_level * n_bins;
    {
        ...
        LOG(DEBUG) << "number of splits = " << n_split;
        LOG(DEBUG) << "best rank & gain = " << best_idx_gain;
    }
    return;
}
...
void HistTreeBuilder::merge_histograms_server_propose(SyncArray<GHPair> &merged_hist, SyncArray<GHPair> &merged_missing_gh) {
    ...
    CHECK_EQ(parties_hist[0].size(), parties_hist[1].size()); // assert that all histograms have the same size
...
//    SyncArray<GHPair> merged_missing_gh(n_size);
    ...
    for (int i = 0; i < parties_hist.size(); i++) { // parties_hist.size() = number of parties
        ...
        // hist_file.open("server_messages/hist_" + std::to_string(i) + ".txt", std::ios::app);
        // hist_file << "====================" << std::endl;
        // for (int j = 0; j < n_bins; j++) {
        //     hist_file << hist_data[j].h << " " << hist_data[j].g << std::endl;
        // }
...
#pragma omp parallel for
    ...
    for (int i = 0; i < parties_missing_gh.size(); i++) { // parties_missing_gh.size() = number of parties
        ...
        // save missing_gh into a file
        std::ofstream missing_gh_file;
        missing_gh_file.open("server_messages/missing_gh_" + std::to_string(i) + ".txt", std::ios::app);
        missing_gh_file << "====================" << std::endl;
        for (int j = 0; j < n_size; j++) {
            missing_gh_file << missing_gh_data[j].h << " " << missing_gh_data[j].g << std::endl;
        }
#pragma omp parallel for
        for (int j = 0; j < n_size; j++) {
