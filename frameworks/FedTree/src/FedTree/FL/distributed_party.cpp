...
void DistributedParty::GetRangeAndSet(int n_bins) {
    ...
    // range_file.open("client_messages/range_" + std::to_string(pid) + ".txt", std::ios::app); // app = append, out = write (default)
    // for (int i = 0; i < feature_range.size(); i++) {
    //     range_file << feature_range[i][0] << " " << feature_range[i][1] << std::endl;
    // }
    // range_file << "====================" << std::endl;
    // range_file.close();
}
...
void DistributedParty::GetRootNode() {
    ...
    // save root node to a file
    // std::ofstream root_file;
    // root_file.open("client_messages/root_" + std::to_string(pid) + ".txt", std::ios::app);
    // root_file << "final_id: " << root.final_id << " lch_index: " << root.lch_index << " rch_index: " << root.rch_index << " parent_index: " << root.parent_index << " gain: " << root.gain << " base_weight: " << root.base_weight << " split_feature_id: " << root.split_feature_id << " pid: " << root.pid << " split_value: " << root.split_value << " split_bid: " << root.split_bid << " default_right: " << root.default_right << " is_leaf: " << root.is_leaf << " is_valid: " << root.is_valid << " is_pruned: " << root.is_pruned << " sum_gh_pair [g, h]: " << root.sum_gh_pair.g << " " << root.sum_gh_pair.h << " n_instances: " << root.n_instances << std::endl;
    // root_file.close();
}
...
void DistributedParty::GetSplitPoints() {
    ...
    // LOG(INFO) << "Saving split points to a file";
    // std::ofstream sp_file;
    // sp_file.open("client_messages/sp_" + std::to_string(pid) + ".txt", std::ios::app);
    // for (auto & sp_point : sp_points) {
    //     sp_file << "gain: " << sp_point.gain << " fea_missing_gh [g, h]: " << sp_point.fea_missing_gh.g << " " << sp_point.fea_missing_gh.h << " rch_sum_gh [g, h]: " << sp_point.rch_sum_gh.g << " " << sp_point.rch_sum_gh.h << " default_right: " << sp_point.default_right << " nid: " << sp_point.nid << " split_fea_id: " << sp_point.split_fea_id << " fval: " << sp_point.fval << " split_bid: " << static_cast<int>(sp_point.split_bid) << " no_split_value_update: " << sp_point.no_split_value_update << std::endl;
    // }
    // sp_file << "====================" << std::endl;
    // sp_file.close();
...
//        party.booster.fbuilder->cut.get_cut_points_fast(party.dataset, n_bins, party.dataset.n_instances());
    ...
    for (int i = 0; i < fl_param.gbdt_param.n_trees; i++) {
        ...
        party.booster.update_gradients();
        if (fl_param.privacy_tech == "dp") {
            const auto gradient_data = party.booster.gradients.host_data();
            // min gradient from gradient_data
            // float_type min_gradient = std::numeric_limits<float_type>::max();
            // float_type max_gradient = std::numeric_limits<float_type>::min();
            // for(int j = 0; j < party.booster.gradients.size(); j++){
            //     min_gradient = std::min(min_gradient, gradient_data[j].g);
            //     max_gradient = std::max(max_gradient, gradient_data[j].g);
            // }
            for(int j = 0; j < party.booster.gradients.size(); j++){
                dp_manager.add_noise_clip_gradient(gradient_data[j].g); //, min_gradient, max_gradient);
            }
        }
